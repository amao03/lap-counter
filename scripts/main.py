...
# cap = cv2.VideoCapture("rtsp://admin :@192.168.1.117/Preview_01_main")
...
# loop over the set of tracked points
    for i in np.arange(1, len(pts)):
        ...
        if counter >= 10 and i == 1 and pts[-10] is not None:
            ...
            if np.abs(dX) > 75 and args["vertical"]:
    ...
    cv2.putText(frame, str(laps), (100, 30), cv2.FONT_HERSHEY_SIMPLEX,
        0.65, (0, 0, 255), 3)
    # cv2.putText(frame, str(round(time.time() - 0,2)), (200, 30), cv2.FONT_HERSHEY_SIMPLEX,
    #     0.65, (0, 0, 255), 3)
    
    startY = 30
    for i in splits:
        cv2.putText(frame, str(round(i,2)), (300, startY), cv2.FONT_HERSHEY_SIMPLEX,
        0.45, (0, 0, 255), 1)
        startY = startY + 20
